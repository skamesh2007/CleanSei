...
function StatRow({ stat }: { stat: ActivityStat }) {
  ...
  const theme = stat.theme ?? {
    iconBg: "bg-zinc-100 dark:bg-muted",
    barColor: "bg-primary",
    badgeBg: "bg-zinc-100 dark:bg-muted",
    badgeText: "text-zinc-600 dark:text-muted-foreground",
  };
  ...
  return (
    <div className="flex items-center gap-3.5 px-4 py-3.5 hover:bg-gray-50 dark:hover:bg-muted/50 transition-colors">
      ...
      {/* Icon */}
      <div
        className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${theme.iconBg}`}
      >
        {stat.icon}
      </div>
      ...
      <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 ml-2">
            <Tooltip>
              <TooltipTrigger asChild>
                <Badge
                  variant="outline"
                  className={`text-[10px] font-semibold px-2 py-0 h-5 border-0 ${theme.badgeBg} ${theme.badgeText}`}
                >
                  {stat.delta}
                </Badge>
              </TooltipTrigger>
        ...
        <Progress
          value={pct}
          className="h-1 bg-gray-100 dark:bg-muted"
        >
          <div className={theme.barColor} />
        </Progress>
      </div>
    </div>
  );
